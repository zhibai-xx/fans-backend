...
export class RateLimitGuard implements CanActivate {
  ...
  // 违规记录
  private readonly violationRecords = new Map<
    string,
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (this.isBlocked(clientKey)) {
      this.logger.warn(
        `被封禁的客户端尝试访问: ${clientKey}, IP: ${this.getClientIp(request)}`,
      );
    ...
    if (!allowed) {
      ...
      if (this.shouldBanClient(clientKey)) {
        ...
        this.logger.warn(
          `客户端因多次违规被封禁: ${clientKey}, IP: ${this.getClientIp(request)}`,
        );
      }
  ...
  private getRateLimitConfig(context: ExecutionContext): RateLimitConfig {
    // 可以通过装饰器自定义配置
    const customConfig = this.reflector.get<Partial<RateLimitConfig>>(
      'rateLimit',
      context.getHandler(),
    );
  ...
  private banClient(clientKey: string): void {
    ...
    this.logger.warn(
      `客户端被封禁: ${clientKey}, 封禁时长: ${this.maliciousIpConfig.banDuration / 1000}秒`,
    );
  }
