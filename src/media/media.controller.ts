...
export class MediaController {
  ...
  async findAll(
    ...
    const userUuidMapping =
      await this.userUuidService.getUuidMappingByIds(userIds);

    // 转换响应数据
    const mediaList = result.data.map(
      (media) => new MediaResponseDto(media, userUuidMapping[media.user_id]),
    );
  ...
  @HttpCode(HttpStatus.NO_CONTENT)
  async deleteMedia(@Param('id') id: string, @Req() req: RequestWithUser) {
  ...
  async getMediaForReview(
    ...
    this.logger.log(
      `获取待审核媒体列表: ${JSON.stringify(filters)}`,
      MediaController.name,
    );

    const result = await this.mediaService.getMediaForReview(
      filters,
      this.userUuidService,
    );
  ...
  async batchUpdateStatus(
    ...
    const adminId = req.user.id;
    this.logger.log(
      `管理员 ${adminId} 批量更新媒体状态: ${dto.mediaIds.length} 个媒体 -> ${dto.status}`,
      MediaController.name,
    );
  ...
  async batchUpdateTags(
    ...
    const adminId = req.user.id;
    this.logger.log(
      `管理员 ${adminId} 批量更新媒体标签: ${dto.mediaIds.length} 个媒体`,
      MediaController.name,
    );
  ...
  async batchUpdateCategory(
    ...
    const adminId = req.user.id;
    this.logger.log(
      `管理员 ${adminId} 批量更新媒体分类: ${dto.mediaIds.length} 个媒体 -> ${dto.categoryId || '无分类'}`,
      MediaController.name,
    );
  ...
  async updateMediaInfo(
    ...
    const adminId = req.user.id;
    this.logger.log(
      `管理员 ${adminId} 更新媒体信息: ${id}`,
      MediaController.name,
    );

    const result = await this.mediaService.updateMediaInfo(
      id,
      updateMediaDto,
      adminId,
    );
