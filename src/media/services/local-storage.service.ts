...
export class LocalStorageService implements IStorageService {
  ...
  constructor(private configService: ConfigService) {
    // 获取上传目录配置
    this.uploadDir = this.configService.get<string>('upload.local.dest', './uploads');
  ...
  async uploadFile(file: Express.Multer.File, customPath?: string): Promise<string> {
    ...
    // 返回可访问的URL路径（这里假设有一个API前缀'/api/media/file/'）
    return `/api/media/file/${relativePath.replace(/\\/g, '/')}`;
  }
  ...
  async deleteFile(fileUrl: string): Promise<boolean> {
    try {
      // 从URL中提取文件路径
      const urlPath = fileUrl.replace('/api/media/file/', '');
