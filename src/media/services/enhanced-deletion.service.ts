...
export class EnhancedDeletionService {
  ...
  async enhancedDelete(
    ...
    for (const mediaId of mediaIds) {
      try {
        const result = await this.deleteMediaWithValidation(mediaId, adminId, options);
        results.push(result);

        if (result.success) {
          totalFilesCleanedUp += Object.values(result.filesDeleted).filter(Boolean).length;
        }
      } catch (error) {
        results.push({
          success: false,
    ...
    this.logger.log(`✅ 删除操作完成: ${summary.successfulDeletions}/${summary.totalRequested} 成功`);
    return summary;
  }
  ...
  private async deleteMediaWithValidation(
    ...
    if (media.thumbnail_url) {
      result.filesDeleted.thumbnail = await storageService.deleteFile(media.thumbnail_url);
    }
    ...
    if (media.media_type === 'VIDEO') {
      result.filesDeleted.processedFiles = await this.cleanupAndVerifyProcessedFiles(mediaId);
    }
   ...
   * 清理处理文件并验证结果
   */
  private async cleanupAndVerifyProcessedFiles(mediaId: string): Promise<boolean> {
    try {
  ...
  private async logDeletion(
    ...
    const logFile = path.join(logDir, `deletion-${new Date().toISOString().split('T')[0]}.log`);
   ...
   * 获取待硬删除的媒体列表
   */
  async getPendingHardDeletion(): Promise<any[]> {
    // 这里需要根据你的数据库结构调整
    // 假设你有 deleted_at 和 scheduled_hard_delete_at 字段
    return [];
  }
   ...
   * 执行定期硬删除任务
   */
  async performScheduledHardDeletion(): Promise<DeletionSummary> {
    const pendingDeletions = await this.getPendingHardDeletion();
    ...
    return this.enhancedDelete(mediaIds, 0, {
